...
export default function WeeklyReport() {
  ...
  useEffect(() => {
    api
      .get<WeeklyReportData>("/ai/report/weekly")
      .then(setReport)
  ...
  const maxCal = Math.max(
    ...daily_data.map((d) => d.calories),
    targets.calories,
    1,
  );

  const adherenceColor =
    summary.adherence_pct >= 90
      ? "bg-green-500"
      : summary.adherence_pct >= 70
        ? "bg-yellow-500"
        : "bg-red-500";
        ...
        <CardContent>
          ...
          <div className="flex items-end gap-2 h-40 pt-2 pb-1">
            ...
            <div className="relative w-full flex items-end gap-2 h-full">
              {daily_data.map((d) => {
                ...
                const atTarget =
                  Math.abs(d.calories - targets.calories) / targets.calories <
                  0.1;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`w-full h-full rounded-t-sm ${barColor}`}
                      />
                    </div>
                    <span className="text-xs font-medium">
                      {DAY_ABBR[d.day_name] || d.day_name}
                    </span>
                  </div>
                );
              })}
          ...
          <div className="flex gap-3 text-xs text-muted-foreground mt-2">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded bg-green-400 inline-block" /> On
              target (±10%)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded bg-yellow-400 inline-block" />{" "}
              Over target
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded bg-primary/70 inline-block" />{" "}
              Under target
            </span>
          </div>
        </CardContent>
          ...
          {[
            ...
            <div key={m.label} className="space-y-1">
              ...
              <Progress
                value={Math.min(
                  100,
                  m.target > 0 ? (m.avg / m.target) * 100 : 0,
                )}
                className="h-2"
              />
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {daily_data.map((d) => (
            <div
              key={d.date}
              className={`flex items-center justify-between py-2 border-b last:border-b-0 ${d.meal_count === 0 ? "opacity-50" : ""}`}
            >
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                {d.meal_count > 0 ? (
                  <>
                    ...
                    {d.exercise_minutes > 0 && (
                      <span>🏋️ {d.exercise_minutes}min</span>
                    )}
