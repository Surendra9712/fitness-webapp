        ...
        <div className="space-y-3">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Pay in person — admin confirms after receiving
              </p>
            </div>
          </button>
        </div>
...
export default function Subscription() {
  ...
  async function handleMethodSelect(method: SubscriptionPaymentMethod) {
    try {
      ...
      await refreshUser();
      toast.success(
        "Pro plan requested. Admin will verify after receiving payment.",
      );
    } catch (e: any) {
  ...
  return (
    <>
      ...
      {esewaData && (
        <EsewaAutoForm url={esewaData.url} params={esewaData.params} />
      )}
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {/* Pro plan */}
          <Card
            className={`relative ${isPro ? "ring-2 ring-primary" : isPending ? "ring-2 ring-amber-400" : ""}`}
          >
            {isPro && (
            ...
            {isPending && (
              <Badge className="absolute -top-2.5 left-4 bg-amber-400 text-white">
                Pending Approval
              </Badge>
            )}
            <CardHeader className="pb-3">
