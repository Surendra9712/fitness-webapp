...
import {
  ...
  ClipboardList, UserCheck, LogOut, Leaf,
} from 'lucide-react'
...
const navLinks: Record<Role, NavItem[]> = {
  ...
  dietitian: [
    { to: '/dietitian',        label: 'Dashboard',  icon: <LayoutDashboard className="h-4 w-4" /> },
    { to: '/dietitian/plans',  label: 'Diet Plans', icon: <ClipboardList className="h-4 w-4" /> },
    { to: '/dietitian/assign', label: 'Assign',     icon: <UserCheck className="h-4 w-4" /> },
  ],
  user: [
    { to: '/user',              label: 'Dashboard',    icon: <LayoutDashboard className="h-4 w-4" /> },
    { to: '/user/my-plan',      label: 'My Plan',      icon: <ClipboardList className="h-4 w-4" /> },
    { to: '/user/log-meal',     label: 'Log Meal',     icon: <Salad className="h-4 w-4" /> },
    { to: '/user/log-exercise', label: 'Log Exercise', icon: <Dumbbell className="h-4 w-4" /> },
  ],
}
...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex h-14 items-center gap-1 border-b bg-emerald-900 px-4 shadow-md">
      {/* Brand */}
      <NavLink to="/" className="mr-6 flex items-center gap-2 text-white">
